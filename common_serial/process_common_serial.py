# -*- coding:utf-8 -*-
'''
file    : process_common_serial
author  : weihaoysgs@gmail.com
des     : $
date    : 2021-08-30 21:06
IDE     : PyCharm
'''
import sys
...
from common_serial.common_serial_ui import Ui_CommonSerial


class CommonSerial(QtWidgets.QWidget, Ui_CommonSerial):
    def __init__(self):
        super(CommonSerial, self).__init__()
        self.setupUi(self)
    ...
    def _init_common_serial_(self):
        ...
        # 定时接受
        # TODO multithreading receive
        self.timer_receive.timeout.connect(lambda :self.receive_data(self.textBrowser_receive_region))
        ''' If you want to change you receive data show plugin, Please remember to change blow slot function '''
        ...
        self.bt_open_com.clicked.connect(self.com_open)  # 注意，在槽函数中关联时不要带括号，否则会一直执行,或者说一开始就会触发该信号
...
if __name__ == '__main__':
    CommonSerialApp = QtWidgets.QApplication(sys.argv)
    COMMON_SERIAL = CommonSerial()
    COMMON_SERIAL.show()
    sys.exit(CommonSerialApp.exec_())
